...
> **Working copy lives in `~/Public/DY.github.io`.** macOS blocks tool access to
> `~/Documents` and `~/Desktop`; the copy still sitting in `~/Documents/DY.github.io`
...
```bash
cd ~/Public/DY.github.io && python3 -m http.server 8765
```
...
dots — if you typed `DY.github.io` meaning the site address, your username is `DY`.)

1. Create a **public** repo on GitHub named `DY.github.io`. Don't add a README —
   this folder already has one.
2. From this folder:

```bash
git init -b main && git add -A && git commit -m "Personal site" && git remote add origin https://github.com/DY/DY.github.io.git && git push -u origin main
```

3. Wait ~1 minute. The site goes live at <https://DY.github.io> — for
